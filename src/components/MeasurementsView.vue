<template>
  <div class="space-y-6">
    ...
    <div v-if="store.measurements.length === 0" class="glass-card p-10 rounded-3xl text-center flex flex-col items-center justify-center">
      ...
      <h3 class="text-lg font-semibold text-white mb-1">No Measurements Yet</h3>
      <p class="text-sm text-gray-400 max-w-xs mb-6">Log your body measurements to see your progress here.</p>
      <button
      ...
      >
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
            <div v-if="log.waist">
              <div class="text-xs text-gray-400">Waist</div>
              <div class="text-sm font-bold text-indigo-400 flex items-baseline gap-1">
                <span>{{ Number(log.waist).toFixed(1) }} <span class="text-[10px] font-normal text-gray-400">cm</span></span>
              </div>
            </div>
            <div v-if="log.chest">
              <div class="text-xs text-gray-400">Chest</div>
              <div class="text-sm font-bold text-sky-400 flex items-baseline gap-1">
                <span>{{ Number(log.chest).toFixed(1) }} <span class="text-[10px] font-normal text-gray-500">cm</span></span>
              </div>
            </div>
            <div v-if="log.arms">
              <div class="text-xs text-gray-400">Arms</div>
              <div class="text-sm font-bold text-emerald-400 flex items-baseline gap-1">
                <span>{{ Number(log.arms).toFixed(1) }} <span class="text-[10px] font-normal text-gray-400">cm</span></span>
              </div>
            </div>
            <div v-if="log.thighs">
              <div class="text-xs text-gray-400">Thighs</div>
              <div class="text-sm font-bold text-amber-500 flex items-baseline gap-1">
                <span>{{ Number(log.thighs).toFixed(1) }} <span class="text-[10px] font-normal text-gray-500">cm</span></span>
              </div>
            </div>
          </div>
        </div>
